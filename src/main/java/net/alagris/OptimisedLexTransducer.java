...
//            debug.accept("Checking ambiguity " + (System.currentTimeMillis() - ambiguityCheckingBegin) + " miliseconds");
    ...
    public static final <N, G extends IntermediateGraph<Pos, E, P, N>> ReplCommand<N, G, String> replIsDeterministic() {
        return (compiler, logs, debug, args) -> {
            try {
                RangedGraph<Pos, Integer, E, P> r = compiler.getOptimisedTransducer(args);
                if (r == null)
    ...
    public static final <N, G extends IntermediateGraph<Pos, E, P, N>> ReplCommand<N, G, String> replListPipes() {
        return (compiler, logs, debug, args) -> {
    ...
    public static final <N, G extends IntermediateGraph<Pos, E, P, N>> ReplCommand<N, G, String> replEqual() {
        return (compiler, logs, debug, args) -> {
            try {
                final String[] parts = args.split("\\s+", 2);
                if (parts.length != 2)
    ...
    public static final <N, G extends IntermediateGraph<Pos, E, P, N>> ReplCommand<N, G, String> replRandSample() {
        return (compiler, logs, debug, args) -> {
            try {
    ...
    public static final <N, G extends IntermediateGraph<Pos, E, P, N>> ReplCommand<N, G, String> replVisualize() {
        return (compiler, logs, debug, args) -> {
            try {

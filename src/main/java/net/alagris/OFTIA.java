...
//    }
//
//    private static Informant informant(ArrayList<Pair<IntSeq, IntSeq>> informant) {
//        Informant prev = null;
//        for (int i = informant.size() - 1; i >= 0; i--) {
//            final Pair<IntSeq, IntSeq> sample = informant.get(i);
//            prev = new Informant(sample.getFirst(), sample.getSecond(), prev);
//        }
//        return prev;
//    }
//
...
//     */
//    private static void buildPttOnward(State ptt, Informant is, int outputOffset, int inputOffset) {
//        if (is.in.size() == inputOffset
//                && is.out.size() == outputOffset) {
//            ptt.isAccepting = true;
//        }
//
//        while (is != null) {
//            assert is.out.sub(0, outputOffset).equals(is.next.out.sub(0, outputOffset));
//            assert is.in.sub(0, inputOffset).equals(is.next.in.sub(0, inputOffset));
//            int symbolIn = is.in.get(inputOffset);
//            if (is.in.size() == inputOffset + 1) {
//                final IntQueue out = asQueue(is.out, outputOffset, is.out.size());
//                Informant filterIn = null;
//                Informant next = null;
//                Informant filterOut = is.next;
//                while () {
//                    if(is.out.sub(outputOffset, is.out.size()) && is.in.get(inputOffset) == symbolIn){
//
//                    }
//                }
//                final State target = ptt.put(symbolIn, out);
...
//        informant.sort((a, b) -> {
//            final int c = a.getFirst().compareTo(b.getFirst());
//            if (c == 0)
//                throw new IllegalArgumentException("Informant breaks functionality (" + a + ") (" + b + ")");
//            return c;
//        });
//
//        buildPttOnward(root, informant(informant), 0, 0);
//        return root;
//    }
//
//

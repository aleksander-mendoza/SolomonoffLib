...
mod tests {
    ...
    fn test_6() {
        Ghost::with_mock(|ghost| {
            let mut g:G = solomonoff::MealyUnionParser::new().parse(ghost,"[bcd]:'re'").unwrap().unwrap();
            let r = optimise_graph(&g, ghost);
            let mut state_to_index = r.make_state_to_index_table();
            let mut output_buffer = Vec::<A>::with_capacity(256);
            unsafe{output_buffer.set_len(256)};
    ...
    fn test_7() {
        Ghost::with_mock(|ghost| {
            let mut g:G = solomonoff::MealyUnionParser::new().parse(ghost,"[b-d]:'re'").unwrap().unwrap();
            let r = optimise_graph(&g, ghost);
            let mut state_to_index = r.make_state_to_index_table();
            let mut output_buffer = Vec::<A>::with_capacity(256);
            unsafe{output_buffer.set_len(256)};
